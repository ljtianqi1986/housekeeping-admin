 ...
 */
public class Pay17OrderRefund {

    private String shop_code;//商户门店标识
    private String order_code;//商户订单号
    private String user_code;//操作员
    private String out_refund_no;//商户退款单号
    private int refund_fee;//退款金额：分

    private String none_str=CreateNoncestr();//随机字符串
    private String version= ZkNode.getIstance().getJsonConfig().get("version").toString();//接口版本号
    private String dxy_key=ZkNode.getIstance().getJsonConfig().get("pay17key").toString();
    private String sign;

    public Pay17OrderRefund() {
        super();
    }

    public String StitchingParam() throws Exception {
        ...
        jsonObject.put("out_refund_no", this.out_refund_no);
        jsonObject.put("refund_fee", this.refund_fee);
        jsonObject.put("none_str", this.none_str);
        jsonObject.put("version", this.version);
        SignUtil.getSign(jsonObject,dxy_key);
        jsonObject.put("sign", sign);
        String cert_sign = RSAUtil.encrypttoStr(ConfigUtil.get("CERT_URL"), out_refund_no);
        jsonObject.put("cert_sign", cert_sign);
