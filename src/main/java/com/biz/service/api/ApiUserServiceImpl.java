...
public class ApiUserServiceImpl extends BaseServiceImpl<TorderMain> implements ApiUserServiceI  {
...
//    protected Logger logger = Logger.getLogger(this.getClass());
    ...
    public void updateBsseUserPhone(User user) throws Exception {
        dao.update("QueryClerkDao.updateBaseUserPhone",user);
    }
    ...
    @Override
    public BaseUser getBaseUserByunionId(String unionId) throws Exception {
        return (BaseUser) dao.findForObject("ApiUserDao.getBaseUserByunionId", unionId);
    }
...
//        return jsonObject;
    ...
    //读取用户信息，如果没注册，则自动注册
    private BaseUser adduser(String unionId) throws Exception {
        //加券接口
        BaseUser y_user =getBaseUserByunionId(unionId);
        //新用户
        if(y_user == null){
            y_user = add_new_user(unionId);
        }

        return y_user;
    }

    public BaseUser add_new_user(String unionId){
        UserInfo userInfo = WechatAccessToken.getUserInfo(unionId);
    ...
    public List<Base90Detail> selectOrtherBalanceIn(Map<String, Object> map) throws Exception {
        return (List<Base90Detail>) dao.findForList("BaseUserDao.selectOrtherBalanceInByunionId", map);
    }

    @Override
    public String int_sum(Map<String, Object> map) throws Exception {
        Object o = dao.findForObject("BaseUserDao.int_sumByUnionId", map);
        return (String) (o != null ? o : 0);
    }

    @Override
    public List<Base90Detail> selectOrtherBalanceOut(Map<String, Object> map) throws Exception {
        return (List<Base90Detail>) dao.findForList("BaseUserDao.selectOrtherBalanceOutByunionId", map);
    }

    @Override
    public String out_sum(Map<String, Object> map) throws Exception {
        Object o = dao.findForObject("BaseUserDao.out_sumByunionId", map);
        return (String) (o != null ? o : 0);
    }

    @Override
    public void updateBaseUserOnlyCode(Map<String, Object> map) throws Exception {
        dao.update("BaseUserDao.updateBaseUserOnlyCodeByUnioId", map);
    }

    @Override
    public List<Base90Detail> selectWhereBase90Detail(Map<String, Object> map) throws Exception {
        return (List<Base90Detail>) dao.findForList("BaseUserDao.selectWhereBase90DetailByunionId", map);
    }
    ...
    @Override
    public PBaseUser getUserByUnionId(String unionId) throws Exception {
        return (PBaseUser) dao.findForObject("BaseUserDao.getUserByUnionId",unionId);
    }

    @Override
    public void giveCoinByCardId(PBaseUser user, String card_code) throws Exception {
        try{
            //第一步查询该卡是否存在赠送比例
            Map<String,Object> mapCoin = (Map<String,Object>)dao.findForObject("offlineCardDao.getCardPercentage",card_code);
            //如果存在，获取比例
            if(mapCoin!=null && mapCoin.get("percentage") !=null && mapCoin.get("cardTotal") != null)
            {
                double percentage_ = Double.valueOf(mapCoin.get("percentage").toString()).doubleValue();
                if(percentage_ > 0)//满足赠送
                {
                    double total_ = Double.valueOf(mapCoin.get("cardTotal").toString()).doubleValue();

                    BigDecimal a1 = new BigDecimal(Double.valueOf(percentage_));
                    BigDecimal b1 = new BigDecimal(Double.valueOf(total_));
                    BigDecimal result = a1.multiply(b1);// 相乘结果
                    BigDecimal one = new BigDecimal("10000");
                    double giveCoin = result.divide(one,2,BigDecimal.ROUND_HALF_UP).doubleValue();//保留2位数
                    System.out.println(giveCoin);
                    //根据比例计算金额并调用接口发放

                    String url=ConfigUtil.get("QT_Url")+"/api/balance/operUserCoin90.ac";
                    Map<String,String > map =new java.util.HashMap<>();
                    Map<String,Object > map1 =new java.util.HashMap<>();
                    map1.put("userId",user.getId());
                    map1.put("state","1");
                    map1.put("orderNum",card_code);
                    map1.put("source","3");
                    map1.put("amount",""+giveCoin);
                    ...
                    Map<String,Object > mapres =new java.util.HashMap<>();
                    mapres.put("flag",map.get("flag").toString());
                    mapres.put("msg",map.get("msg").toString());
                }
            }
        }catch (Exception e){
            e.printStackTrace();
